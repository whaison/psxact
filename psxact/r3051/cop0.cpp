...
uint32_t Cop0::FetchSr(uint32_t reg) {
  return this->registers[reg] & registers_mask[reg];
}

void Cop0::StoreSr(uint32_t reg, uint32_t data) {
  this->registers[reg] = data & registers_mask[reg];
}

uint32_t Cop0::FetchCr(uint32_t reg) {
  return 0;
}

void Cop0::StoreCr(uint32_t reg, uint32_t data) {
}
