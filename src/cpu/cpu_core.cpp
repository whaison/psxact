...
void core_t::main() {
  ...
  while (1) {
    ...
    case 0x00: // special
      ...
      case 0x1a: // div rs,rt
        if (regs.u[decoder::rt()]) {
          regs.lo = uint32_t(regs.i[decoder::rs()] / regs.i[decoder::rt()]);
          regs.hi = uint32_t(regs.i[decoder::rs()] % regs.i[decoder::rt()]);
        }
        else {
      ...
      case 0x2a: // slt rd,rs,rt
        regs.i[decoder::rd()] = regs.i[decoder::rs()] < regs.i[decoder::rt()];
        continue;

      case 0x2b: // sltu rd,rs,rt
        regs.i[decoder::rd()] = regs.u[decoder::rs()] < regs.u[decoder::rt()];
        continue;
      }
      break;
    ...
    case 0x0a: // slti rt,rs,$nnnn
      regs.i[decoder::rt()] = regs.i[decoder::rs()] < decoder::iconst();
      continue;

    case 0x0b: // sltiu rt,rs,$nnnn
      regs.i[decoder::rt()] = regs.u[decoder::rs()] < uint32_t(decoder::iconst());
      continue;
...
void core_t::enter_exception(uint32_t code, uint32_t epc) {
  uint32_t sr = cop0.regs[12];
  sr = (sr & ~0x3f) | ((sr << 2) & 0x3f);

  uint32_t cause = cop0.regs[13];
  cause = (cause & ~0x3f) | ((code << 2) & 0x3f);
