  ...
  while (1) {
    ...
    switch (decoder::op_hi()) {
      ...
      case 0x20: // lb rt,$nnnn(rs)
        state.registers.i[decoder::rt()] = int8_t(
            read_data(BYTE, state.registers.u[decoder::rs()] + decoder::iconst()));
        continue;

      case 0x21: // lh rt,$nnnn(rs)
        state.registers.i[decoder::rt()] = int16_t(
            read_data(HALF, state.registers.u[decoder::rs()] + decoder::iconst()));
        continue;
